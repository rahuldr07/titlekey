...
import { LinkMonitor } from '@/routes/LinkMonitor'
...
import {
  ClientDetail, Denied, Integrations, Intake, LeadDetail, MyPerf, NewLead,
  NewOrder, Onboard, Person, RepGen, SignIn,
} from '@/routes/misc'
...
const ordersIndexRoute = createRoute({
  getParentRoute: () => ordersRoute, path: '/', component: Orders,
...
const leadsRoute = createRoute({
  getParentRoute: () => rootRoute, path: '/leads', component: () => <Outlet />,
})
...
const payslipsRoute = createRoute({
  getParentRoute: () => rootRoute, path: '/payslips', component: () => <Outlet />,
})
...
const reportsRoute = createRoute({
  getParentRoute: () => rootRoute, path: '/reports', component: Reports,
...
const companyRoute = createRoute({
  getParentRoute: () => rootRoute, path: '/company', component: () => <Outlet />,
})
const companyIndexRoute = createRoute({
  getParentRoute: () => companyRoute, path: '/', component: Company,
  validateSearch: (s: Record<string, unknown>): { tab?: string } => ({
    tab: typeof s.tab === 'string' ? s.tab : undefined,
  }),
})
...
/* ── Alias routes ─────────────────────────────────────────────────────────────
   The original's ALIAS map resolves these to a TAB of another screen. That is
   how the ZIP behaves, so the behaviour is preserved rather than the pages
   being dropped: each alias redirects to its host screen with the tab selected. */
